...
import { removeLedgerByTransaction } from '../utils/savingsTargets';
...
export default function Transactions({ openModal, onModalStateChange }) {
  ...
  const savingsTargetMap = useMemo(() => {
    const m = {};
    savingsTargets.forEach((t) => { m[t.id] = t; });
    return m;
  }, [savingsTargets]);
  ...
  const openEdit = (tx) => {
    // Savings transfers are generated by the savings-target ledger; editing
    // them would desync the target progress. Use the Tabungan page instead.
    if (tx.savingsTargetId) {
      toast.error('Transaksi tabungan diubah dari halaman Tabungan');
      return;
    }
    // Subscription expenses are generated by the asset scheduler; editing them
    // would desync the billing schedule. Use the Aset page instead.
    if (tx.assetId) {
      toast.error('Transaksi langganan diubah dari halaman Aset');
      return;
    }
    setEditTx(tx);
    setTxType(tx.type);
  ...
  const handleSave = async () => {
    ...
    if (txType === 'transfer') {
      ...
      if (isWalletLocked(wallets, form.fromWalletId)) {
        toast.error(lockedWalletMessage(wallets, form.fromWalletId, 'transfer keluar'));
        return;
      }
      ...
      if (isWalletLocked(wallets, form.walletId)) {
        toast.error(lockedWalletMessage(wallets, form.walletId, 'pengeluaran'));
        return;
      }
      ...
      if (isWalletLocked(wallets, form.walletId)) {
        toast.error(lockedWalletMessage(wallets, form.walletId, 'hutang'));
        return;
      }
      const walletBalance = walletBalances[form.walletId] || 0;
  ...
  const handleDelete = async () => {
    ...
    // Keep the savings ledger in sync when a savings transfer is deleted
    if (tx?.savingsTargetId) {
      await removeLedgerByTransaction(deleteId);
    }

    // Rolling back a subscription charge restores its previous due date,
    // otherwise deleting an accidental charge would skip a whole period.
    if (tx?.assetId) {
      const asset = await db.assets.get(tx.assetId);
      if (asset && asset.lastChargeTransactionId === deleteId && asset.nextDueDate) {
        const rolledBack = new Date(asset.nextDueDate);
        rolledBack.setMonth(
          rolledBack.getMonth() - (asset.billingCycle === 'yearly' ? 12 : 1),
        );
        await db.assets.update(asset.id, {
          nextDueDate: rolledBack.toISOString(),
          lastChargeTransactionId: null,
          lastChargedAt: null,
  ...
  const handleSettleDebt = async (tx) => {
    // Debt settlement is incoming money, so a locked wallet may still receive it.
    const now = new Date().toISOString();
            ...
            <div key={dateKey}>
              ...
              <div className="space-y-2">
                {txList.map((tx) => {
                  ...
                  const savingsTarget = savingsTargetMap[tx.savingsTargetId];
                  ...
                  if (isDebt) {
                    ...
                    txIcon = asset.icon || '🔄';
                    txBgColor = (asset.color || '#6366f1') + '20';
                    txLabel = asset.name;
                    txSubtext = tx.note || (wallet?.name || '');
                  } else if (tx.type === 'transfer') {
                    txIcon = '↔️';
                    if (savingsTarget) {
                      txIcon = savingsTarget.icon || '🎯';
                      txBgColor = (savingsTarget.color || '#10b981') + '20';
                    }
                  }
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0" onClick={() => { if (!isRepayment) openEdit(tx); }}>
                          <div className="flex items-center gap-1.5">
                            ...
                            {savingsTarget && (
                              <span className="text-[9px] font-medium text-emerald-400 bg-emerald-500/15 px-1.5 py-0.5 rounded-full shrink-0 flex items-center gap-0.5">
                                <Lock size={8} /> Tabungan
                              </span>
                            )}
                            {asset && (
                              <span className="text-[9px] font-medium text-primary-400 bg-primary-500/15 px-1.5 py-0.5 rounded-full shrink-0 flex items-center gap-0.5">
                                <RefreshCw size={8} /> Langganan
                              </span>
                            )}
                          </div>
             ...
             <>
               <div className="input-group">
                 ...
                 >
                    ...
                    {wallets.map((w) => (
                      <option key={w.id} value={w.id}>
                        {w.isLocked ? '🔒 ' : ''}{w.icon} {w.name} {txType === 'income' ? '' : `(${formatIDR(walletBalances[w.id] || 0)})`}
                      </option>
                    ))}
                  </select>
                </div>
